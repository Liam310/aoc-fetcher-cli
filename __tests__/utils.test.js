...
describe('fetchRawAdventData', () => {
  ...
  const DAY_1_URL = `https://adventofcode.com/${VALID_YEAR}/day/${VALID_DAY}/input`;
  const VALID_AUTH_HEADERS = { cookie: 'fake-cookie' };
  const FAKE_DATA = '123\n456\n789\n';
  const INVALID_DAY = 27;
  const INVALID_YEAR = 2450;
  ...
  test('invokes fetch with the correct URL and auth headers', async () => {
    await fetchRawAdventData(VALID_YEAR, VALID_DAY, VALID_AUTH_HEADERS);
    expect(fetch).toHaveBeenCalledWith(DAY_1_URL, {
      headers: VALID_AUTH_HEADERS,
    });
  });
  test('returns raw data from given day', async () => {
    const data = await fetchRawAdventData(
      VALID_YEAR,
      VALID_DAY,
      VALID_AUTH_HEADERS
    );
  ...
  test('responds with fetch error given invalid day', async () => {
    await expect(
      fetchRawAdventData(VALID_YEAR, INVALID_DAY, VALID_AUTH_HEADERS)
    ).rejects.toEqual(FETCH_ERROR_404_MSG);
  });
  test('responds with fetch error given invalid year', async () => {
    await expect(
      fetchRawAdventData(INVALID_YEAR, VALID_DAY, VALID_AUTH_HEADERS)
    ).rejects.toEqual(FETCH_ERROR_404_MSG);
  });
  test('responds with custom error given bad auth', async () => {
    await expect(
      fetchRawAdventData(VALID_YEAR, VALID_DAY, INVALID_AUTH_HEADERS)
...
describe('writeAndLog', () => {
  ...
  test('passes values onto data fetcher function', async () => {
    mockFetchData.mockResolvedValue(FAKE_DATA);

    await writeAndLog(FILE_PATH, YEAR, DAY, AUTH_HEADERS);
    expect(mockFetchData).toHaveBeenCalledWith(YEAR, DAY, AUTH_HEADERS);
